...
<script type="text/javascript">
	...
	function optDelete(name,isDisplay) {
		...
		//执行批量删除
		$("#jvForm").attr("action","deletes.do?name="+name+"&isDisplay="+isDisplay);
...
<div class="body-box">
<form action="/control/brand/list.do" method="post" style="padding-top:5px;">
品牌名称: <input type="text" value="${name }" name="name"/>
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${pagination.list}" var="entry">
			<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
				<td align="center"><input type="checkbox" value="${entry.id}" name="ids"/></td>
				...
				<td align="center">
				<a class="pn-opt" href="javascript:void(0)" onclick="window.location.href='toEdit.do?id=${entry.id}'">修改</a> | <a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;} window.location.href='delete.do?id=${entry.id}&name=${name}&isDisplay=${isDisplay}'" href="javascript:void(0)">删除</a>
				</td>
			</tr>
		</c:forEach>
	</tbody>
