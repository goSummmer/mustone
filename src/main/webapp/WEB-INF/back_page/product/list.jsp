...
function isShow(pageNo,name,brandId,isShow) {
	...
	$("#jvForm").attr("action","isShow.do?name=" + name+ "&brandId=" + brandId + "&isShow=" + isShow+"&page="+pageNo);
	$("#jvForm").submit();
}
</script>
</head>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${productListWithPage.list }" var="entry">
			<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
				<td align="center"><input type="checkbox" name="ids" value="${entry.id }"/></td>
...
<div class="page pb15">
	<span class="r inb_a page_b">
		<c:forEach items="${productListWithPage.pageView }" var="page">
			${page }
		</c:forEach>
	</span>
</div>
<div style="margin-top:15px;"><input class="del-button" type="button" value="删除" onclick="optDelete();"/><input class="add" type="button" value="上架" onclick="isShow('${productListWithPage.pageNo}','${name}','${brandId}','${isShow}');"/><input class="del-button" type="button" value="下架" onclick="optDelete();"/></div>
</form>
</div>
</body>
</html>